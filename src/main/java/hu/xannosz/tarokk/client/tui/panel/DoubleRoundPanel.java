...
public class DoubleRoundPanel extends Panel {
    public DoubleRoundPanel(DoubleRoundType doubleRoundTypeIn) {
        for (DoubleRoundType doubleRoundType : DoubleRoundType.values()) {
            if (doubleRoundType.equals(doubleRoundTypeIn)) {
                addComponent(new Label(doubleRoundType.getName()).setTheme(ThemeHandler.getHighLightedThemeMainPanel()));
            } else {
