...
public class DataMetaPanel extends Panel {
    public DataMetaPanel(TuiClient tuiClient, MainProto.GameSession gameData) {
        ...
        if (!Util.anyNull(tuiClient.getServerLiveData().getGameType(),
            ...
            addData(this, Translator.INST.beginnerPlayer, getPlayerName(tuiClient.getServerLiveData().getBeginnerPlayer(), gameData, tuiClient.getServerLiveData()));
            addData(this, Translator.INST.phase, tuiClient.getServerLiveData().getPhase().getName());
            addData(this, Translator.INST.playerInTurn, getPlayerName(tuiClient.getServerLiveData().getPlayerTurn(), gameData, tuiClient.getServerLiveData()));
            ...
            if (tuiClient.getServerLiveData().getPlayerActions().get(Actions.FOLD) != null) {
                for (Douplet<Integer, String> folded : tuiClient.getServerLiveData().getPlayerActions().get(Actions.FOLD)) {
                    addData(this, getPlayerName(folded.getFirst(), gameData, tuiClient.getServerLiveData()) + Translator.INST.foldedTarock, getFormattedCardName(folded.getSecond()));
                }
            }

            for (
                    Map.Entry<Integer, Integer> skartedTarock : tuiClient.getServerLiveData().getSkartedTarocks().entrySet()) {
                addData(this, getPlayerName(skartedTarock.getKey(), gameData, tuiClient.getServerLiveData()) + Translator.INST.foldedTarock, "" + skartedTarock.getValue());
            }
            ...
            if (tuiClient.getServerLiveData().getPlayerActions().get(Actions.CALL) != null) {
                for (Douplet<Integer, String> call : tuiClient.getServerLiveData().getPlayerActions().get(Actions.CALL)) {
                    addData(this, getPlayerName(call.getFirst(), gameData, tuiClient.getServerLiveData()) + Translator.INST.calledTarock, getFormattedCardName(call.getSecond()));
                }
            }

            for (Map.Entry<Integer, Integer> entry : tuiClient.getServerLiveData().getCardsTakenUsers().entrySet()) {
                addData(this, getPlayerName(entry.getKey(), gameData, tuiClient.getServerLiveData()) + Translator.INST.takenCards, "" + entry.getValue());
            }
        }
    }
}
